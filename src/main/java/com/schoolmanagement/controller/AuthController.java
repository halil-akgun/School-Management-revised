...
//        String token = "Bearer " + jwtUtils.generateJwtToken(authentication);//bu sekilde de olur,farketmez

        UserDetailsImpl userDetailsImpl = (UserDetailsImpl) authentication.getPrincipal();

        Set<String> roles = userDetailsImpl.getAuthorities()
                .stream()
                .map(GrantedAuthority::getAuthority) // rolleri otomatik stringe cevirir
                .collect(Collectors.toSet());

        Optional<String> role = roles.stream().findFirst();

        // farkli bir yontem: burada obje once bos olusuyor, sonra setleniyor
        AuthResponse.AuthResponseBuilder authResponse = AuthResponse.builder();
        authResponse.username(userDetailsImpl.getUsername());
        authResponse.token(token);
        authResponse.name(userDetailsImpl.getName());
        ...
        if (role.isPresent()) {
            ...
            if (role.get().equalsIgnoreCase(RoleType.TEACHER.name())) {
                authResponse.isAdvisor(userDetailsImpl.getIsAdvisor().toString());
            }
        }
